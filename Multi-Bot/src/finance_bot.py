...
from utils import *
def get_prompttemplate_financebot(code_suboption):
...
def get_conversational_chain_financebot(code_suboption):
    prompt_template=get_prompttemplate_financebot(code_suboption)
    prompt = prepare_prompt_gen(prompt_template)
    model = get_model_gen()
    chain = LLMChain(llm=model, prompt=prompt)
    return chain

def user_input_financebot(user_question,code_suboption):
    chain = get_conversational_chain_financebot(code_suboption)
    response = chain(
